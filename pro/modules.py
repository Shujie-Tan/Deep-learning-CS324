...
class Linear(object):
    def __init__(self, in_features, out_features):
        ...
        """
        self.weight = np.random.randn(out_features, in_features) * 0.0001   # 0.0001?
    ...
    def backward(self, dout):
        ...
        """
        self.grads['bias'] = np.matmul(np.eye(len(self.bias)), dout)
        ...
        self.grads['weight'] = np.matmul(self.grads['weight'], dout)

        dx = np.matmul(np.transpose(self.weight), dout)
        return dx
